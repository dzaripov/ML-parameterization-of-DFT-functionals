...
class MLOptimizer(nn.Module):
    ...
    def custom_sigmoid(self, x):
        ...
        # from 0 to 1
        result = (1 + torch.e + (torch.e - 3) / 3) / (
            1 + (torch.e - 3) / 3 + torch.e ** (-0.5 * x + 1)
        )
        return result
    ...
    def forward(self, x):
        ...
        if self.DFT == "SVWN":  # constraint for VWN3's Q_vwn function to 4*c - b**2 > 0
            ...
            for b_ind, c_ind in zip((2, 3, 11, 12, 13), (4, 5, 14, 15, 16)):
                constants.append(
                    torch.abs(x[:, c_ind]) + (x[:, b_ind] ** 2) / 4 + 1e-5
                )  # torch.abs or nn.ReLU()?
            x = torch.cat(
                [
                    x[:, 0:4],
                    torch.stack(constants[0:2], dim=1),
                    x[:, 6:14],
                    torch.stack(constants[2:], dim=1),
                    x[:, 17:],
                ],
                dim=1,
            )
        ...
        if self.DFT == "PBE":
            ...
            if x.shape[1] == 2:
                x = torch.cat(
                    [
                        torch.ones(x.shape[0], 4).to(device),
                        x,
                        torch.ones(x.shape[0], 18).to(device),
                    ],
                    dim=1,
                )
