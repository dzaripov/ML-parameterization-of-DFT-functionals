...
# c_arr[:, 18:21] equal params_a_a 
# c_arr[:, 21:24] equal params_a_alpha1 
...
def g_aux(k, rs, c_arr):
    res_g_aux = c_arr[:, 3:6][:, k]*torch.sqrt(rs) 
    + c_arr[:, 6:9][:, k]*rs
    + c_arr[:, 9:12][:, k]*rs**1.5
    + c_arr[:, 12:15][:, k]*rs**(c_arr[:, 15:18][:, k] + 1)
    catch_nan(res_g_aux=res_g_aux)
    return res_g_aux


def g(k, rs, c_arr):
    log = torch.log(1 +  1/(2*c_arr[:, 18:21][:, k]*g_aux(k, rs, c_arr)))
    res_g = -2*c_arr[:, 18:21][:, k]*(1 + c_arr[:, 21:24][:, k]*rs) * log
    catch_nan(res_g=res_g, log=log)
    return res_g 


def f_pw(rs, z, c_arr):
    res_f_pw = g(0, rs, c_arr) + z**4*f_zeta(z)*(g(1, rs, c_arr) 
    - g(0, rs, c_arr) + g(2, rs, c_arr)/c_arr[:, 2]) 
    - f_zeta(z)*g(2, rs, c_arr)/c_arr[:, 2]
    catch_nan(res_f_pw=res_f_pw)
    return res_f_pw
    

def A(rs, z, t, c_arr):
    res_A = c_arr[:, 0]/(c_arr[:, 1]*(torch.exp(-f_pw(rs, z, c_arr)/(c_arr[:, 1]*mphi(z)**3)) - 1))
    catch_nan(res_A=res_A)
    return res_A
...
def f2(rs, z, t, c_arr):
    ...
    res_f2 = c_arr[:, 0]*f1_/(c_arr[:, 1]*(A_+1)*f1_ + eps)
    catch_nan(res_f2=res_f2, f1_=f1_, A_=A_)
    return res_f2


def fH(rs, z, t, c_arr):
    log = torch.log(1+f2(rs, z, t, c_arr))
    res_fH = c_arr[:, 1]*mphi(z)**3*log
    catch_nan(res_fH=res_fH, log=log)
    return res_fH


def PBE_C(rs, z, xt, c_arr):
    res_PBE_C = f_pw(rs, z, c_arr) + fH(rs, z, tt(rs, z, xt), c_arr)
...
def lda_x_spin(rs, z, c_arr):
    ...
    res_lda_x_spin = c_arr[:, 24]*(z+1)**(1 + 1/DIMENSIONS)*2**(-1-1/DIMENSIONS)*rs_f_rs
    catch_nan(res_lda_x_spin=res_lda_x_spin, const=c_arr[:, 24], rs_f_rs=rs_f_rs)
...
# c_arr[:, 25] equals params_a_kappa 
# c_arr[:, 26] equals params_a_mu 
...
def pbe_f0(s, c_arr):
    res_pbe_f0 = 1 + c_arr[:, 25]*(1 - c_arr[:, 25]/(c_arr[:, 25] + c_arr[:, 26]*s**2))
