...
def xs_xt_calc(rho, sigmas):  # sigma 1 is alpha beta contracted gradient
    ...
    eps_add = 1e-10
    DIMENSIONS = 3
    xs0 = torch.sqrt(sigmas[:, 0] + eps_add**2) / (rho[:, 0] + eps_add) ** (
        1 + 1 / DIMENSIONS
    )
    # xs1 = torch.sqrt(sigmas[:,2])/(rho[:,1]+eps)**(1 + 1/DIMENSIONS)
    # eps and 1e-19 added
    xs1 = torch.where(
        (sigmas[:, 2] < eps) & (rho[:, 1] < eps),  # last sigma and last rho equal 0
        torch.sqrt(sigmas[:, 0] + eps_add**2)
        / (rho[:, 0] + eps_add) ** (1 + 1 / DIMENSIONS),
        torch.sqrt(sigmas[:, 2] + eps_add**2)
        / (rho[:, 1] + eps_add) ** (1 + 1 / DIMENSIONS),
    )
    xt = torch.sqrt(sigmas[:, 0] + 2 * sigmas[:, 1] + sigmas[:, 2] + eps_add**2) / (
        rho[:, 0] + rho[:, 1] + eps_add
    ) ** (1 + 1 / DIMENSIONS)
...
def f_pw(rs, z, c_arr):
    res_f_pw = (
        ...
        + z**4
        * f_zeta(z)
        * (g(1, rs, c_arr) - g(0, rs, c_arr) + g(2, rs, c_arr) / c_arr[:, 2])
...
def A(rs, z, t, c_arr, device):
    ...
    expm1 = torch.expm1(
        torch.where(-f_pw_ / mphi_ < const_87, -f_pw_ / mphi_, const_87)
    )
    res_A = c_arr[:, 0] / (c_arr[:, 1] * expm1)
    catch_nan(
        res_A=res_A, f_pw_=f_pw_, mphi_=mphi_, expm1=expm1, rs=rs, z=z, c_arr=c_arr
    )
    return res_A
...
def fH(rs, z, t, c_arr, device):
    ...
    log = torch.where(
        f2_ <= -1, torch.log1p(f2_ + eps), torch.log1p(f2_)
    )  # weird infinity
...
def gga_exchange(func, rs, z, xs0, xs1, c_arr):  # -screen_dens -z_thr
    res_gga_exchange = lda_x_spin(rs, z, c_arr) * func(xs0, c_arr) + lda_x_spin(
        rs, -z, c_arr
    ) * func(xs1, c_arr)
