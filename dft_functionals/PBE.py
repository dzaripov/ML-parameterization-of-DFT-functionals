...
def xs_xt_calc(rho, sigmas, t=True):  # sigma 1 is alpha beta contracted gradient
    ...
    xs0 = torch.sqrt(sigmas[:, 0] + eps_add_sigma) / (rho[:, 0] + eps_add_rho) ** (
        1 + 1 / DIMENSIONS
    )
    xs1 = torch.where(
        (sigmas[:, 2] < eps) & (rho[:, 1] < eps),  # last sigma and last rho equal 0
        torch.sqrt(sigmas[:, 0] + eps_add_sigma)
        / (rho[:, 0] + eps_add_rho) ** (1 + 1 / DIMENSIONS),
        torch.sqrt(sigmas[:, 2] + eps_add_sigma)
        / (rho[:, 1] + eps_add_rho) ** (1 + 1 / DIMENSIONS),
    )

    if t:
        xt = torch.sqrt(
            sigmas[:, 0] + 2 * sigmas[:, 1] + sigmas[:, 2] + eps_add_sigma
        ) / (rho[:, 0] + rho[:, 1] + eps_add_rho) ** (1 + 1 / DIMENSIONS)
...
def A(rs, z, t, c_arr, device):
    ...
    expm1 = torch.expm1(
        torch.where(
            -f_pw_ / mphi_ < 87, -f_pw_ / mphi_, -f_pw_ / mphi_ + f_pw_ / mphi_ + 87
        )
    )
